...
| ArchUnit `layeredArchitecture()` | ❌ — 7 production packages (`cli`, `configuration`, `eckey`, `keyproducer`, `opencl`, `persistence`, root); the real candidate | ✅ `e673471` (this session) — but as a narrower `argsPackageIsALeaf` rule, not the full layered form. Evidence-based verdict: a clean 3-layer `Args → Json → Api` was attempted and failed because `json` parsers/serializers genuinely depend on root-package DTOs (`Pair`, `ChatMessage`, `ContentPart`) AND the root API genuinely consumes `json` parsers — they are peers in the public API layer, not a stackable hierarchy. The only real architectural invariant the current package design supports is "args stays a leaf"; the new rule pins that. `noPackageCycles` already covered the looser "no cycles between subpackages" property. | ➖ single-package (all production code in `aiindex`) | ➖ single-package |

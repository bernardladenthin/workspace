...
- **Gradle-based signing-key verify (`verify-signing-key-gradle`) — java-llama.cpp only.** The shared `verify-signing-key` job (see "In parity" above) uses the **`gpg` CLI**, which faithfully mirrors **maven-gpg-plugin** — how all 4 repos sign their *Maven* artifacts. jllama is the **only** repo that *also* signs via **Gradle's `signing` plugin + `useInMemoryPgpKeys` (BouncyCastle)**: the `llama-android` AAR publish (`publishAllPublicationsToCentralSnapshotsRepository` / `…StagingRepository`, `llama-android/build.gradle.kts`). That BouncyCastle path is **stricter than `gpg`** at parsing the armored key and is exactly where the AAR snapshot signing failed (`Cannot invoke "org.bouncycastle.openpgp.PGPPrivateKey.getPublicKeyPacket()" because "<parameter1>" is null` — a null private key, i.e. the in-memory key/passphrase could not be unlocked, even though the *same* key signs fine via `gpg`). So jllama gets a **second, Gradle-based self-test** that drives `useInMemoryPgpKeys` on a throwaway signing project — the only faithful test of the path that actually broke. The three pure-Maven siblings (BAF, sb, plugin) have **no** Gradle signing path, so the extra job would exercise a mechanism they never use — hence jllama-only, same shape as the Depot/sccache and pocl/OpenCL entries. Branch `claude/android-signing-failure-q7zml9`. **Resolved 2026-07-09 (jllama `main`, run 29012094281):** root cause was Gradle's **2-arg** `useInMemoryPgpKeys` selecting the **primary** key, whose secret this BouncyCastle can't unlock, while `gpg`/maven-gpg-plugin auto-select the key's **4096-bit signing subkey** `07D2D767`. Fix: both Gradle signing setups (the `.github/signing-selftest/` harness **and** `llama-android/build.gradle.kts`) switch to the **3-arg** `useInMemoryPgpKeys(keyId, key, passphrase)` when `MAVEN_GPG_KEY_ID` is set, driven by a new **`GPG_KEY_ID`** env secret (= `07D2D767`, a public key id) added to the `maven-central` environment in all 4 repos (consumed only by jllama; the Maven siblings' gpg agent already picks the subkey). `verify-signing-key-gradle` is green on `main`, so the `llama-android` AAR publish — identical code + same secret — signs correctly. The gpg harness was refactored from a base64 blob to a committed, readable project at `.github/signing-selftest/`. PR #307.
...
| Test-JVM diagnostics + memory standard ([`policies/ci-test-diagnostics.md`](policies/ci-test-diagnostics.md)): `-Xmx2g`/no eager `-Xms`, `-XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=. -XX:ErrorFile=hs_err_pid%p.log`, CI `free -h` before/after + `mvn -e` + on-failure crash-dump upload (`hs_err`/`*.hprof`/surefire `*.dump`/`*.dumpstream`/`*.txt`/`TEST-*.xml`). Divergences kept by design: `reuseForks`/`forkCount`/`forkedProcessTimeoutInSeconds` (BAF), `--add-opens` set (BAF), pocl job (BAF), core dumps (jllama). | 🚧 argLine `-XX` flags appended; `test` matrix memory steps (Linux-guarded) + `*.hprof` in uploads; pocl `*.hprof` | 🚧 `-Xmx2g` added (already had `-XX` + memory steps + crash upload); `mvn -e` added | 🚧 new surefire `<argLine>@{argLine} …>` + memory steps + crash upload | 🚧 new surefire `<argLine>@{argLine} …>` + memory steps + crash upload |

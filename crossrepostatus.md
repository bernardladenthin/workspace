...
- BAF persistence implementations all in place: `HashSetAddressPresence`, `TruncatedLong64SortedArrayPresence`, `BloomFilterAccelerator`, `AddressLookupBackend` enum, `AddressPresence` + `AddressLookup` interfaces. Benchmark **now JMH** at `benchmark/AddressLookupBenchmark.java` (`7b69278`, this session — predecessor JUnit timing test `persistence/AddressLookupBenchmarkTest.java` deleted in the same commit; the predecessor's commit body explicitly said "JUnit comparison benchmark (not JMH)" and admitted it sidestepped the JMH harness even though the harness existed). `HashSetPrecomputedHashAddressPresence` does NOT exist yet. `GridSizeSweepBenchmark` IS present in `benchmark/`.
...
- BAF dead-package removal: `eckey/` package + `Secp256k1.java` (195 lines, hand-ported scalar-mul from a 2014 StackOverflow answer) + `Secp256k1Test.java` (84-line non-asserting `System.out.println` demo) deleted in `a5911c6` after investigation confirmed the file was a development-time reference oracle for the OpenCL secp256k1 kernel — superseded by the production tests using bitcoinj's `ECKey.fromPrivate` as the oracle. The dead `eckeyIsLowLevelCrypto` ArchUnit rule was dropped in the same commit (replaced with an inline comment pointing at the commit so the rule can be restored from history if a future low-level crypto helper appears); `module-info.java`'s broken `exports …eckey;` was fixed and the missing `exports …constants;` was added.
- BAF post-leaf-extraction literal cleanup (`2949daa`): three pre-existing files now route through the constants leaf instead of inlining literals. `PrivateKeyTooLargeException.java` uses `Radix.HEX` for `toString(…)`; `SecretsFile.java` uses `Radix.HEX` for `new BigInteger(_, …)` in both `STRING_DO_SHA256` and `SHA256` branches; `AddressTxtLine.java` line 256 uses `OpenClKernelConstants.RIPEMD160_HASH_NUM_BYTES` instead of the stray `new byte[20]` literal (the same file already used the named constant at lines 145 + 166). `Secp256k1.java`'s similar smells were rendered moot by the file's deletion in `a5911c6`.
- BAF test-execution policy (`c985275`): CLAUDE.md `## Testing` section gains a `### Test execution policy for AI assistants` subsection that pins "narrow scope by default; full surefire only when the user explicitly asks". Recipes per change type (constants/leaf extraction → `BitcoinAddressFinderArchitectureTest`; refactor inside one class → `<ThatClass>Test`; config/wire-format change → `ConfigFixturesParseTest` + the touched POJO test; arch-rule edit → arch test only; OpenCL/LMDB touchups → matching integration test). Worth replicating into the other three repos' CLAUDE.md when their test suites grow large enough to make the policy useful — currently BAF is the only repo where a full `mvn test` is slow enough (~5 min) to justify the discipline.
...
| **`-Werror` flipped** | ✅ **`2881c96`** (this session) | ✅ | ✅ | ✅ | ✅ parity reached — `-Werror` is on across all 4 repos. |
...
4. ✅ **workspace**: parity refresh (`e211548`).
5. ✅ **BAF `-Werror` flipped** in 2 commits:
    - `f37f162` — moved 14 `@NonNull`/`@Nullable` annotations from before to after modifiers (Checker Framework `[type.anno.before.modifier]` cleanup). Pure mechanical fix across 8 files.
    - `2881c96` — final 5 `sun.misc.Unsafe` proprietary-API warnings eliminated by **deleting `ByteBufferUtility#freeByteBuffer` entirely**. Investigation showed the warning category is immune to `@SuppressWarnings`, `-XDignore.symbol.file=true`, and every `-Xlint:-` flag (verified empirically under `--release 21`). The Unsafe usage was already a no-op on OpenJ9 / GraalVM Native Image / Android per the file's own Javadoc — those JVMs already relied on the JVM's built-in `Cleaner` to reclaim direct buffers when they become unreachable. HotSpot now joins them: no eager free, only reachability-based reclamation. `<arg>-Werror</arg>` is on; 386 tests across the touched surfaces (ByteBufferUtility, ConsumerJava, ProducerOpenCL, ProbeAddressesOpenCL gated, Finder, AbstractPlaintextFile, Address* and persistence/* helpers, architecture rules) pass.
...
| `javac -Werror` + `-Xlint:all,-serial,-options` | ✅ **`2881c96`** (this session) — `<arg>-Werror</arg>` is on with `-Xlint:all,-serial,-options,-classfile,-processing` (matching the 3 sibling repos byte-for-byte). Path: blockers 1–6 cleared in `2d99c4a`; 14 Checker Framework `[type.anno.before.modifier]` cleared in `f37f162`; last 5 `sun.misc.Unsafe` proprietary-API warnings eliminated in `2881c96` by deleting `ByteBufferUtility#freeByteBuffer` (the eager invokeCleaner path was already a no-op on OpenJ9/GraalVM/Android per its own Javadoc — HotSpot now joins them via the JVM's built-in Cleaner). | ✅ `3e2efbb` (verified 2026-06-04, line 366; `-Xlint:all,-serial,-options,-classfile,-processing`; compile clean under -Werror) | ✅ **`f7cf748`** (verified 2026-06-04, line 341; same flags; compile clean) | ✅ `7a4fbf0` (verified 2026-06-04, line 351; same flags; compile clean) |
...
| ArchUnit `layeredArchitecture()` | ✅ `bd58221` + `ca75527` + `95d2d6f` + `a5911c6` (this session) — **3 ArchUnit rules** (was 4 before the `eckey` deletion): `constantsPackageIsALeaf`, `configurationDoesNotDependOnRuntimeLayers` (strict — bans root entirely), `cliIsEntryPointOnly`. The 4th rule (`eckeyIsLowLevelCrypto`) was dropped in `a5911c6` when the `eckey` package itself was deleted: investigation showed it housed a single dead file (`Secp256k1.java`, a development-time reference oracle for the OpenCL secp256k1 scalar-mul kernel — zero production callers, the lone test was a non-asserting `System.out.println` demo). After three leaf extractions (`Secp256k1Constants` `ca75527`, `Radix` `3fd0206`, `OpenClKernelConstants` `95d2d6f`) plus the `CProducer.getOverallWorkSize(BitHelper)` inlining (`ca75527`), the `configuration → root` edge count is **zero**: configuration POJOs only import from `constants/`. A new `OpenClKernelConstantsTest` (10 tests) pins the 60+ byte-layout values against accidental edits (CHUNK_SIZE_NUM_BYTES=104, MAXIMUM_CHUNK_ELEMENTS=20 648 881, BIT_COUNT_FOR_MAX_CHUNKS_ARRAY=24, …). | ✅ `e673471` (this session) — as `argsPackageIsALeaf` (narrower). `json` parsers/serializers and root API are peers (each side depends on the other via DTOs / parser usage), not stackable. Full layered form requires splitting DTOs into a dedicated `value/` package, which breaks published public-API FQNs — see `policies/code-quality-todos.md` §4. | ➖ single-package (all production code in `aiindex`) | ➖ single-package |
...
| Package-architecture refactor + full `layeredArchitecture()` rule (see `policies/code-quality-todos.md` §4) | ❌ — last `configuration → root` edge closed in `95d2d6f` via the `OpenClKernelConstants` leaf extraction. Strict `configurationDoesNotDependOnRuntimeLayers` rule now bans root entirely. Remaining work for full `Architectures.layeredArchitecture()`: split root package into `core` (DTOs) and `orchestration` (Finder, Producer*, Consumer*) — strictly bigger move because it touches public-API class FQNs. | ❌ — needs DTO split into `value/` package (breaks public-API FQNs) | ➖ single-package | ➖ single-package |
...
| Persistence-backends research/implementation | ✅ HashSet snapshot, BloomFilterAccelerator extraction, TRUNCATED_LONG_64, AddressLookupBackend config enum, AddressPresence/AddressLookup chain contract, 4 stale `loadToMemoryCacheOnInit` example JSONs cleaned up (`3c4d558`), **JMH migration of `AddressLookupBenchmarkTest` → `AddressLookupBenchmark` (`7b69278`, this session)**. Remaining open: open-addressing hash table backend; standalone Bloom-only backend; `HashSetPrecomputedHashAddressPresence` (Pre-compute HASHSET hash item). | ➖ | ➖ | ➖ |
...
- Persistence follow-ups (re-verified 2026-06-04): ~~4 stale `loadToMemoryCacheOnInit` example JSONs~~ ✅ DONE this session; ~~JMH migration of `AddressLookupBenchmarkTest`~~ ✅ DONE this session (`7b69278`); open-addressing hash table backend; standalone `BloomFilterPersistence`; `HashSetPrecomputedHashAddressPresence` (Pre-compute HASHSET hash item). Default backend remains `BLOOM`.
...
- Feature-investigation first batch (UTF-8 boundary decoder + ~~per-run timing line~~ + jbang example + ~~system-properties README table~~) — 2 of 4 batch items now done. **`system-properties README table` done in `e36f631`** (full 9-row deep-scan; corrected the "All ... resolved by LlamaSystemProperties" claim into a per-property scope/consumer breakdown) plus the two registry-bypass cleanups it surfaced: `OSInfo.getArchName()` routes through `LlamaSystemProperties.getOsinfoArchitecture()` (`3ae6c81`); dead `getLibName()` getter + the `LlamaLoader` `lib.name` Javadoc lie deleted (`28dc9e6`) after a forensic comparison against `kherud/java-llama.cpp` upstream proved the code path that originally honoured `lib.name` was removed in upstream commit `6bb63e1` when the loader was extended to load multiple shared libraries (ggml + jllama). **`per-run timing line` done in `3248c1c`** — new `TimingsLogger` utility class with `format(Timings)` + `log(Timings)`; wired into both non-streaming result parsers (`CompletionResponseParser`, `ChatResponseParser`) right after the `Timings` instance is built; format mirrors `llama.cpp` CLI (`prompt: N tok in X ms (Y tok/s) | gen: … | cache: N | draft: …`); dedicated SLF4J logger `net.ladenthin.llama.timings` so users can suppress it independently; 7 unit tests pin format byte-exact + LogCaptor-based pipeline-delivery check. Streaming generation (`LlamaIterable`/`LlamaIterator`) is not yet hooked — no clean public-API "I am done" callback to thread through; separate follow-up. **Remaining first-batch items:** UTF-8 boundary-safe streaming decoder + jbang single-file example.

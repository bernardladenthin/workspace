...
- Feature-investigation first batch (UTF-8 boundary decoder + per-run timing line + jbang example + ~~system-properties README table~~) — ~~~~1-2 days, no JNI changes~~. **`system-properties README table` done in `e36f631` (this session)** — full 9-row deep-scan table in `README.md#system-properties-reference` (was 5 rows; corrected from "All ... resolved by LlamaSystemProperties" to a per-property scope/consumer breakdown). The two registry-bypass smells the scan surfaced as new TODOs are **both also resolved this session**: `OSInfo.getArchName()` now routes through `LlamaSystemProperties.getOsinfoArchitecture()` (`3ae6c81`); the dead `LlamaSystemProperties.getLibName()` getter was deleted along with the `LlamaLoader` Javadoc lie about `lib.name` after a forensic comparison against `kherud/java-llama.cpp` upstream proved the code path that originally honoured `lib.name` was removed in upstream commit `6bb63e1` ("add ggml shared library to binding") when the loader was extended to load multiple shared libraries (`28dc9e6`; README row dropped, fork-side cleanup now matches the actual upstream behaviour and resolves a years-old documentation lie carried over from upstream). Remaining items of the first-batch backlog: UTF-8 boundary-safe streaming decoder + per-run timing line + jbang example.
